...
# Given the string DRIVER_ARGS, split it up and reform as a json
QUIET=${QUIET:-""}
DRIVER_ARGS=${DRIVER_ARGS:-""}
DRIVER_ARGS_JSON=""
if [ -z "$DRIVER_ARGS" ]; then
    if [ -z "$QUIET" ]; then
        DRIVER_ARGS_JSON=', "--log", "debug"'
    fi
else
...
# fi
sed -i "s|@@DRIVER_ARGS@@|$DRIVER_ARGS_JSON|g" /etc/selenoid/browsers.json
